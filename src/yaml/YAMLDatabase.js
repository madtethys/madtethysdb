...
class YAMLDatabase extends EventEmitter {
    constructor(construct) {
        ...
        this.jsonFilePath = construct.filePath || "./madtethysdb/database.yaml"
        this.eventData = {}
        ...
        if (file[file.length - 1] == "yaml" ? false : file[file.length - 1] == "yml" ? false : true) {
            throw Error("❌ madtethysdb Hatası: Girilen veri tabanı dosyasının uzantısı yaml veya yml değil")
        }
        setInterval(() => {
        ...
        if (construct.backup) {
            ...
            this.backup = new Backup({
                path: construct.backup.path || "./madtethysdb/backup.yaml",
            ...
            this.getBackupData = () => {
                let res = {};
                Object.entries(YAML.parse(fs.readFileSync(this.backup.path, "utf-8"))).map(x => Object.entries(x[1].data)).forEach(x => {
                    x.forEach(u => {
                        res[u[0]] = u[1]
                    })
